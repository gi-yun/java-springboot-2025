...
- 연산자는 거의 대부분의 언어에서 동일하게 사용
    ...
    - [Java실습](./Day01/spring01/src/main/java/com/giyun00/spring01/Spring01Application.java)
...
- 조건문 : if문, switch문
    ...
    - 3항 연산자 - 간단하게 구현할 조건문 if-else 대신 사용. 실행코드 블럭이 한 줄일 경우.
        ...
        ```
    - [Java문법실습](./Day01/spring02/src/main/java/com/giyun01/spring02/Spring02Application.java)
- 반복문 : for문, while문 do~while문
    ...
    - for문 - 대표적인 반복문
        ...
        ```
        <br>
    - [Java문법실습](./Day01/spring03/src/main/java/com/giyun03/spring03/Spring03Application.java)
    ...
    - do~while문 - 우선 실행코드를 한번 실행 후 조건을 검사후 반복복
        ...
        ```
    -[Java문법실습](./Day02/spring04/src/main/java/com/giyun00/spring04/Spring04Application.java)
...
- **static**
    ...
    - stactic으로 하면 프로그램 실행 전부터 종료 직전까지 그냥 사용할 수 있는 메서드가 됨.
    - stactic 메서드에서는 같은 클래스 내 일반 메서드는 호출 불가
    - stactic메서드만 호출 가능
    - 다른 클래스는 상관없음
...
- 개요
    ...
    - 클래스 개요
        ...
        class NewCar {
            ...
            public void turnLeft(){ // 기어변동
                
            }
            public void turnRight(){// 기어변동
                
            }
            // 생성자 함수(파이썬__init__유사)
            NewCar(){
                model_name = "없음";
                company = "현대자동차";
            }
...
- 특징
    ...
    ```
- [Java문법실습](./Day02/spring08/src/main/java/com/giyun00/spring08/Spring08Application.java)
...
- 모든 클래스의 조상 클래스. extends를 사용하지 않아도 항상 상속됨.
    - java.lang.Object
    - Object클래스가 가지고 있는 멤버변수와 메서드는 다 사용가능
...
- [Java문법실습](./Day03/spring09/src/main/java/com/giyun00/spring09/Spring09Application.java)
...
- 인터페이스는 상속이라 부르지 않고 구현이라 부름
    ...
    - implements 로 가져오면 인터페이스 내의 추상 메서드는 반드시 구현
...
- 예외를 직접처리하지 않고 상위 클래스나 메서드에게 처리를 인가
    ...
    ```
- [Java문법실습](./Day03/spring10/src/main/java/com/giyun00/spring10/Spring10Application.java)
#### 객체지향 특징
- 추가로 학습할 내용
- 다형성, 상속, 캡슐화, 추상화
...
- String
    - 컴퓨터 -> 숫자, 사람 -> 문자
- [Java문법실습](./Day03/spring12/src/main/java/com/giyun00/spring12/Spring12Application.java)
...
- [Java문법실습](./Day04/spring13/src/main/java/com/giyun00/spring13/Spring13Application.java)

#### 제네릭
- 파이썬과 다르게 java는 단일형만 배열이나 컬렉션에서 사용할 수 있음
- Object 타입으로 지정하면 무슨 형이든 다 할당 가능
- `다양한 타입의 객체들을 다루는 메서드나 컬렉션 클래스를 컴파일 과정에서 안전하게 타입 체크를 해주는 기능`
- 장점
...
- [Java문법실습](./Day04/spring14/src/main/java/com/giyun/spring14/Spring14Application.java)

#### 자료구조
- 다수의 데이터를 저장, 관리하기 위한 목적으로 나온것 - 자료구조
- 배열 : 다수의 데이터를 저장, 관리하기 위해 최초로 개발
- 배열의 단점을 개선 : 리스트, 맵, 튜플, 딕셔너리, 그래프, 트리...
- 배열보다 데이터 사이즈는 커지지만 속도는 개선
- 이런 자료구조를 `Java 컬렉션 프레임워크`라고 지칭

<img src="image/sb002.png" width ="600">

- List : 순서가 있는 데이터 집합. 데이터 중복 허용
- Set : 순서가 없는 데이터. 데이터 중복 허용x
...
- 컬렉션 생성시
    ...
    ```java
    ArrayList<String> list = new AraayList<>(); // 권장안함
...
[Java문법실습](./Day04/spring15/src/main/java/com/giyun/spring15/Spring15Application.java)

#### 람다식
- 함수형 프로그래밍 언어의 특징을 객체지향 언어에 녹여넣은 것
- 익명 함수로 부르기도 함
- 코드의 간겨성 : 코드 구문이 확 줄어듬
...
- [Java문법실습](/Day04/spring16/src/main/java/com/giyun00/spring16/Spring16Application.java)
...
- 리스트와 유사. 문자열 처리 때문에 메모리 문제, 성능문제가 발생하면 StringBuilder, StringBuffer 사용 고려
    ...
    ```
- [Java문법실습](./Day05/spring17/bin/main/com/giyun00/spring17/Spring17Application.class)
...
#### 파일 입출력(File IO)
- 파일을 읽고 쓰는 작업 
- 파일 쓰기 : `FileOutputStream`, `FilWriter`, `PrintWriter` 등 의 클래스 사용
- 입출력 예외가 발생할 수 있기 때문에 클래스나 메서드에 `throws IOException을 추가한다.
- 파일읽기 : `FileInputStream`, `BufferedReader` 등 의 클래스 사용

- [Java문법실습](./Day05/spring18/bin/main/com/giyun00/spring18/Spring18Application.class)
...
##### 1. @Override
- 오버라이드를 올바르게 했는지 컴파일러가 체크
- 상속, 인터페이스 구현시 사용
##### 2. @Deprecated
- 앞으로  다음버전에서 삭제될 수 있음. 사용하지 말것을 권유하는 체크
<img src="./image/sb010.png" width="700">
- 되도록이면 이 함수는 사용하지 말것

##### 3. @FunctionlInterface
- 함수형 인터페이스에 붙여서, 컴파일러가 올바르게 작성되었는지 체크

###### 4. @SuppressWarnings
- 컴파일러의 경고메시지가 표시되지 않음.
    - 실무에선 버전 때문에 뜨는 경고를 지울 때 사용.
